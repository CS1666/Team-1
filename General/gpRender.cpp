...
void gpRender::renderOnScreenEntity2(std::vector<Sprite*> osEntity, std::vector<int> galaxies, std::vector<std::vector<SDL_Rect*> > background1, std::vector<std::vector<SDL_Rect*> > background2, SDL_Rect camera, HpBar hp, bool fixed){
	...
	//SDL_Rect hppos = {hp.getX() + camera.x, hp.getY() + camera.y, hp.getW(), hp.getH()};
	...
	if (!fixed){
		...
		SDL_Rect campos = {hp.getX(), hp.getY(), hp.getW()*(int)hp.getPercentage()/100, hp.getH()};
		...
		SDL_RenderCopy(gRenderer, hp.getTexture(), nullptr, &campos);
