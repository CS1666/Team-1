...
AIShip::AIShip(const AIShip& aiship){
		pathComplete = aiship.pathComplete;
		destination = aiship.destination;
        path = aiship.path;
       	curGoal = aiship.curGoal;//'modes'
}
AIShip::AIShip(SDL_Rect dBox, SDL_Texture* aTex): Ship(dBox, aTex, -1) {renderOrder = 1;};
